...
- 2026-04-27 — 8rf_parallel_demo_orchestrator: bd vla_kitting-8rf. New `scripts/orchestrate/parallel_scripted_demo_gen.sh` + `merge_scripted_shards.py` (with `--delete_after_copy`) + `--seed` plumbed into `scripted_pick_demo.py`. Smoke 2 × 10 demos, 17m12s wall-clock; merge sanity-check passed. Per-demo 144 MB raw HDF5, ~75 MB on LeRobot. Disk math forces v5 production to ≤240 demos at the new 512² + long-phase cost; **going with 200 demos (4×50) for vla_kitting-8ux** to keep safe headroom. Streamed per-shard convert is filed for future > 240-demo runs.
